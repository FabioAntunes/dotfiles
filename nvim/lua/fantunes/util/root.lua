...
---@class UtilRoot
---@field paths string[]
---@field spec UtilRootSpec

---@alias UtilRootFn fun(buf: number): (string|string[])

---@alias UtilRootSpec string|string[]|UtilRootFn
...
function M.realpath(path)
  ...
  return UtilCore.norm(path)
end

---@param spec UtilRootSpec
---@return UtilRootFn
...
---@param opts? { buf?: number, spec?: UtilRootSpec[], all?: boolean }
...
function M.info()
  ...
  lines[#lines + 1] = "```"
  UtilCore.info(lines, { title = "UtilCore Roots" })
...
function M.setup()
  vim.api.nvim_create_user_command("UtilRoot", function()
    UtilCore.root.info()
  end, { desc = "UtilCore roots for the current buffer" })
